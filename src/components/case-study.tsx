...
export function SectionCard({ index, label, title, children }: SectionCardProps) {
  return (
    <section className="group border-t border-white/10 pt-8 sm:pt-10">
      ...
      <div className="space-y-5 text-base leading-8 text-fog sm:text-lg sm:leading-9">{children}</div>
    </section>
  );
}
...
export function SnapshotAside({ highlights, meta }: SnapshotAsideProps) {
  return (
    <aside className="border-t-2 border-volt bg-panel p-6 sm:p-8 lg:sticky lg:top-24">
      ...
      <dl className="mt-8 space-y-4 border-t border-white/10 pt-6">
        {meta.map((row) => (
          <div key={row.label}>
            <dt className="font-mono text-[10px] uppercase tracking-[0.25em] text-fog">{row.label}</dt>
            <dd className="mt-1 font-mono text-xs uppercase tracking-wider text-bone">{row.value}</dd>
          </div>
        ))}
      </dl>
    </aside>
